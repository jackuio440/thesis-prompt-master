...
### 研究材料與方法 / Materials and Methods (Chinese)
```
你是一位學術論文寫作專家，協助撰寫[學門領域]碩士／博士論文。

<context>
[在此貼上你的研究材料與方法細節：研究設計、資料來源、研究工具／材料、實驗或資料收集流程、分析方法、倫理考量、研究領域]
</context>

<task>
根據以上 context 撰寫本論文第三章「研究材料與方法」。
請先從 context 判讀研究領域，依該領域慣例選取適用小節，並調整內容深度。
不適用的小節可省略，但須在該節位置簡述省略原因。

各小節涵蓋要點如下：

1. **研究設計與倫理**
   - 研究典範與設計類型（回溯性／前瞻性、實驗性／觀察性、量化／質性／混合）
   - 倫理審查：IRB 或倫理委員會核准編號 [待補編號]、知情同意或同意書豁免
   - 依領域自適應：臨床研究須說明 IRB 細節；非人體研究若無 IRB 須說明原因

2. **研究材料／資料來源**
   - 來源機構、資料庫或實驗材料名稱與版本
   - 納入與排除條件
   - 樣本數／資料量及其決定依據
   - 依領域自適應：臨床研究報告病人基本資料與標註流程；工程／ML 研究報告資料集規模、標籤來源、資料切分策略（訓練／驗證／測試）；實驗研究報告試劑、儀器廠牌型號與規格參數

3. **研究工具／儀器／材料**
   - 使用的測量工具、儀器、軟體、問卷或實驗耗材
   - 效度、信度或校準資訊（若有）
   - 依領域自適應：影像研究報告取得參數（模態、掃描儀、kVp/mAs/層厚 或 磁場強度/序列）；ML 研究報告模型架構、框架與版本；社會科學報告量表名稱與 Cronbach's α

4. **實驗程序／資料收集流程**
   - 依時間順序描述實施步驟、場域與期程
   - 前處理步驟（若有）：正規化、資料擴增、缺值處理等
   - 依領域自適應：ML 研究報告訓練設定（損失函數、優化器、學習率、batch size、epochs、硬體、隨機種子）；實驗研究報告操作變項控制與盲化設計
...
5. **資料分析方法**
   ...
   - 效果量、信賴區間與顯著水準設定
   - 多重比較校正方式（若有）
   - 依領域自適應：ML 研究報告評估指標（AUROC、Dice、F1 等）及信賴區間估計方法；質性研究報告編碼框架與評估者間一致性

6. **嚴謹性保證**（量化：效度與信度；質性：可信度與可轉移性）
   - 確保研究品質的具體做法
   - 依領域自適應：ML 研究報告外部驗證資料集與可解釋性方法（Grad-CAM、SHAP 等）；臨床研究報告偏誤控制措施
</task>
...
- 語態：以被動語態為主，整章保持一致
- 字數：約 [2500] 字
- 請勿捏造任何數值（樣本數、儀器參數、統計值）；缺少數值時寫 [待補數值]
- 請勿捏造引用文獻；缺少引用時寫 [待補引用]
- 可重現性優先：所述細節須足以讓另一研究者複製本研究
- 依研究領域自適應：醫學影像／ML 研究適用時依 CLAIM 或 TRIPOD-AI 報告結構；臨床試驗適用時依 CONSORT；觀察性研究適用時依 STROBE
- 只產出「研究材料與方法」章節，不得加入結果或討論
</constraints>

<output_format>
含編號小節標題的學術散文，可直接貼入論文。資料集特性或儀器規格可以表格呈現並引用為（見表 X）。缺漏數值標記 [待補數值]，缺漏引用標記 [待補引用]。
</output_format>
```
